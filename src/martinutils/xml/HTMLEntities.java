...
public class HTMLEntities {
	...
	/**
	 * Convert greek characters into numbered XML entities.
	 * @author Martin
	 * @param str
	 * @return
	 */
	public static String greekentities( String str )
	{
		if ( str == null )
			return "";
		...
		for ( int i = 0; i < strlen; i++ )
		{
			ch = str.charAt(i);
			chint = (int)ch;
			if ( chint >= 913 && chint <= 982 ) // is a greek alphabet utf-8 character
				buf.append("&#"+chint+";"); // append xml numbered entity
			else
				buf.append(ch);
		}
	...
	/**
	 * Convert numbered XML entities into greek characters.
	 * @author Alessandro
	 * @param str
	 * @return
	 */
	public static String ungreekentities( String str )
	{
		if ( str == null )
			return "";
		...
		for (chint = 913; chint <= 982; chint++ ) // greek alphabet utf-8 character
		{
			ch = (char)chint;
			String tmp = Character.toString(ch);
			str = str.replace("&#" + chint + ";", tmp);
		}
	 ...
	 */
	public static String htmlentities(String str, boolean convertAmp)
	{
		if (str == null) {
			return "";
		}
	...
	public static String unhtmlentities(String str)
	{
		...
		cleanString = buf.toString();
		
		cleanString = ungreekentities(cleanString);
		cleanString = unhtmlAmpersand(cleanString);
		cleanString = unhtmlAngleBrackets(cleanString);
		cleanString = unhtmlDoubleQuotes(cleanString);
		cleanString = unhtmlQuotes(cleanString);
		cleanString = unhtmlSingleQuotes(cleanString);
